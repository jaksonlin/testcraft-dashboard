        ...
        <div className="p-6 space-y-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            {testCase.type && (
              <div>
                ...
                <p className="text-sm font-medium text-gray-900">{testCase.type}</p>
              </div>
            )}
            {testCase.status && (
              <div>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm font-medium text-gray-900">{testCase.status}</span>
                </div>
              </div>
            )}
          </div>
          ...
          {testCase.setup && (
            <div className="bg-blue-50 rounded-lg p-4">
              ...
              <p className="text-sm text-gray-700 whitespace-pre-wrap">{testCase.setup}</p>
            </div>
          )}
          ...
          <div className="bg-gray-50 rounded-lg p-4">
            ...
            <div className="text-sm text-gray-700 whitespace-pre-wrap">{testCase.steps}</div>
          </div>
          ...
          {testCase.expectedResult && (
            <div className="bg-green-50 rounded-lg p-4">
              ...
              <p className="text-sm text-gray-700 whitespace-pre-wrap">{testCase.expectedResult}</p>
            </div>
          )}
          ...
          {testCase.teardown && (
            <div className="bg-purple-50 rounded-lg p-4">
              <h4 className="font-semibold text-gray-900 mb-2">Teardown / Postcondition</h4>
              <p className="text-sm text-gray-700 whitespace-pre-wrap">{testCase.teardown}</p>
            </div>
          )}
          ...
          {testCase.requirements && testCase.requirements.length > 0 && (
            <div>
              <h4 className="font-semibold text-gray-900 mb-2">Linked Requirements</h4>
              <div className="flex flex-wrap gap-2">
                {testCase.requirements.map((req, idx) => (
                  <span
                    key={idx}
                    className="px-2 py-1 bg-blue-100 text-blue-800 text-xs font-mono rounded border border-blue-300"
                  >
                    {req}
                  </span>
                ))}
          ...
          {testCase.customFields && Object.keys(testCase.customFields).length > 0 && (
            <div>
              <h4 className="font-semibold text-gray-900 mb-2">Additional Fields</h4>
              <div className="bg-gray-50 rounded-lg p-4 space-y-2">
                {Object.entries(testCase.customFields).map(([key, value]) => (
                  <div key={key} className="flex items-start gap-4">
                    <span className="text-sm font-medium text-gray-600 w-32">{key}:</span>
